...
test("tickets 19-20: record, end, continue, split intervals and terminal correction", async () => {
  ...
  detail = await call(handler, `/api/private/sessions/${start.body.session_key}`); const closed = recordFor(detail.body, 4, 8); closed.training_intervals = detail.body.training_intervals.map((interval) => interval.ended_at ? interval : { ...interval, ended_at: new Date().toISOString() });
...
test("tickets 22-23: Coach Share and privacy-filtered export", async () => {
  const { handler, store } = appFixture();
  const created = await call(handler, "/api/private/coach-share", post({}, "share")); assert.equal(created.response.status, 201); assert.equal(created.body.url, undefined);
