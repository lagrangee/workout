...
test("tickets 19-20: record, end, continue, split intervals and terminal correction", async () => {
  ...
  const corrected = await call(handler, `/api/private/sessions/${start.body.session_key}/record`, json({ method: "PUT" }, correction)); assert.equal(corrected.response.status, 200); assert.equal(corrected.body.status, "partial"); assert.equal(corrected.body.completion_fraction, 0.5); assert.ok(corrected.body.training_intervals.every(/** @param {any} interval */ (interval) => interval.ended_at)); assert.equal(canonicalJson(corrected.body.snapshot), beforeSnapshot);
  const restored = await call(handler, `/api/private/sessions/${start.body.session_key}/record`, json({ method: "PUT" }, { ...closed, training_intervals: corrected.body.training_intervals })); assert.equal(restored.response.status, 200); assert.equal(restored.body.status, "completed"); assert.equal(restored.body.completion_fraction, 1); assert.deepEqual(restored.body.training_intervals, corrected.body.training_intervals);
});
...
test("tickets 22-23: Coach Share and privacy-filtered export", async () => {
  ...
  const token = copy.body.url.split("/coach/")[1]; const manifest = await call(handler, `/api/coach/v1/${token}`, { headers: {} }, "ignored@example.invalid"); assert.equal(manifest.response.status, 200); assert.equal(manifest.body.schema_version, 1); assert.equal(manifest.body.api_base_url, `https://workout.example/api/coach/v1/${token}`); assert.equal(manifest.body.manifest_url, manifest.body.api_base_url); assert.equal(manifest.body.schema_catalog_url, "https://workout.example/api/coach/v1/schemas"); assert.equal(manifest.body.endpoints.progress.parameters.bucket.default, "week"); assert.deepEqual(manifest.body.endpoints.progress.parameters.bucket.enum, ["day", "week", "month"]);
  const readme = await call(handler, `/coach/${token}`, { headers: {} }, "ignored@example.invalid"); assert.equal(readme.response.status, 200); assert.match(readme.body, /secret/); assert.match(readme.body, /Coach Share API v1/); assert.match(readme.body, /\/api\/coach\/v1\/schemas/); assert.match(readme.body, /training_version/); assert.match(readme.body, /current_date_may_be_incomplete/); assert.match(readme.body, new RegExp(`API Base URL: https://workout\\.example/api/coach/v1/${token}`));
