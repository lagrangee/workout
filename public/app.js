...
function todayView() {
  ...
  if (entry.kind === "rest") return `<section class="today-page"><div class="today-content"><p class="eyebrow">${state.today.date}</p><span class="status-dot rest"></span><h1>休息日</h1><p class="muted">今天不安排训练。</p><div class="quiet-card">今天把恢复留给自己。</div></div></section>`;
...
function postSessionCommand(detail, command, body = {}, options = {}) {
...
function pauseBoundary(detail, now) {
...
function persistSessionPause(closeAt = null) {
  const detail = state.sessionDetail;
...
function recordsPageHead(active, eyebrow, heading, { subtitle = "", aside = "", backLabel = "", backAction = "" } = {}) {
  const back = backLabel ? `<button class="text-button records-back-button" data-action="${backAction}">${backLabel}</button>` : "";
  const copy = `${back}<p class="eyebrow">${eyebrow}</p><h1>${heading}</h1>${subtitle ? `<p class="muted">${subtitle}</p>` : ""}`;
  return `<section class="page-head records-page-head"><div class="records-page-heading">${copy}</div><div class="records-page-actions">${recordsTabs(active)}${aside}</div></section>`;
}
...
function aerobicActivityCard(activity) {
  ...
  const heartRate = summary.average_heart_rate_bpm == null ? "" : ` · 平均心率 ${summary.average_heart_rate_bpm} bpm`;
  const distance = summary.distance_km == null ? "—" : `${summary.distance_km} km`;
...
function aerobicDetailView(detail) {
  ...
  const distance = summary.distance_km == null ? "—" : `${summary.distance_km} km`;
  return `${recordsPageHead("aerobic", "RECORDS · AEROBIC", "活动详情", { subtitle: `${escapeHtml(detail.local_date || "未知日期")} · ${escapeHtml(aerobicSportLabel(detail.sport_type, detail.sport_name))}`, backLabel: "← 返回有氧记录", backAction: "aerobic-back" })}<section class="aerobic-detail-card"><div class="aerobic-detail-status"><span class="status-pill ${escapeHtml(detail.source_status || "none")}">${escapeHtml(aerobicStatusLabel(detail.source_status))}</span><span>${routeStatus}</span></div><div class="metric-grid aerobic-metrics"><article><span>距离</span><strong>${distance}</strong></article><article><span>用时</span><strong>${aerobicDuration(summary.duration_sec)}</strong></article><article><span>平均心率</span><strong>${summary.average_heart_rate_bpm == null ? "—" : `${summary.average_heart_rate_bpm} bpm`}</strong></article><article><span>消耗</span><strong>${summary.calories_kcal == null ? "—" : `${summary.calories_kcal} kcal`}</strong></article></div><dl class="aerobic-source"><div><dt>活动时间</dt><dd>${activityTime}</dd></div><div><dt>FIT</dt><dd>${escapeHtml(detail.fit_status || "—")}</dd></div></dl>${routeAction ? `<div class="hero-actions route-detail-actions">${routeAction}</div>` : ""}</section>`;
}
...
function routePanelContent() {
  ...
  return `<div class="route-panel-head"><button class="text-button" data-action="routes-close">← 返回有氧记录</button></div><div class="route-list">${routes.length ? routes.map((route) => `<button class="route-list-row" data-action="route-detail" data-route-key="${escapeHtml(route.route_key)}"><span><strong>${escapeHtml(route.route_name || route.route_key)}</strong><small>${route.activity_count || 0} 次活动${route.total_distance_km == null ? "" : ` · ${formatDistanceKm(route.total_distance_km)}`}</small></span><span>›</span></button>`).join("") : `<p class="muted">还没有已确认的路线。</p>`}</div>`;
}
...
function aerobicView() {
  ...
  const head = recordsPageHead("aerobic", "RECORDS · AEROBIC", "有氧", { aside: routeButton });
  ...
  return `${head}${filters}${dateScope ? `<div class="aerobic-date-scope">${dateScope}</div>` : ""}<div class="aerobic-route-layout${routeFocusClass}">${activityPane}${state.aerobic.routesOpen ? routeBrowser() : ""}</div>`;
}
