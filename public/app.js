...
const calendarStatus = (entry, session) => { if (entry.kind === "rest") return { key: "rest", label: "休息日" }; if (entry.kind === "no_plan") return { key: "no_plan", label: "无计划" }; if (session?.status === "in_progress") return { key: "in_progress", label: "进行中" }; if (session?.status === "completed") return { key: "completed", label: "已完成" }; if (session?.status === "partial") return { key: "partial", label: "部分完成" }; if (session?.status === "skipped") return { key: "skipped", label: "已跳过" }; if (entry.is_overdue_unstarted) return { key: "overdue", label: "逾期未开始" }; if (entry.date === state.today?.date) return { key: "today", label: "今日待训练" }; return { key: "scheduled", label: "待训练" }; };
...
function todayView() {
  ...
  if (!entry || entry.kind === "no_plan") return `<section class="today-page"><header class="today-app-header"><span></span><strong>今日</strong><span></span></header><div class="today-content"><p class="eyebrow">${state.today?.date || "今天"}</p><h1>今天没有计划</h1><p class="muted">可以在设置中提交未来训练计划。</p></div></section>`;
  if (entry.kind === "rest") return `<section class="today-page"><header class="today-app-header"><span></span><strong>今日</strong><span></span></header><div class="today-content"><p class="eyebrow">${state.today.date} · ${state.today.timezone}</p><span class="status-dot rest"></span><h1>休息日</h1><p class="muted">今天不安排训练。</p><div class="quiet-card">今天把恢复留给自己。</div></div></section>`;
...
function restView(detail, items) {
  ...
  return `${sessionHeader(detail, false)}${progressDisclosure(detail, items)}<section class="rest-screen"><span class="rest-label">组间休息</span><h2>放松，准备下一项</h2><div class="rest-time" data-rest-remaining aria-live="polite">${formatRestRemaining()}</div><div class="next-context"><span>接下来</span><strong>${escapeHtml(exerciseName(detail, next))}${context.setNumber ? ` · 第 ${context.setNumber} 组` : ""}</strong><small>${escapeHtml(focusTarget(next.target))}</small></div><button class="secondary" data-action="skip-rest">跳过休息</button></section>${sessionFooter(detail, false)}`;
}

function todayProgressView(session, entry, detail) { const items = detail?.snapshot?.completion_items || []; const completed = detail?.completion_results || []; const fraction = detail?.completion_fraction ?? session.completion_fraction ?? 0; const rows = detail ? detail.snapshot.blocks.flatMap((block) => block.exercises).map((exercise) => { const exerciseItems = items.filter((item) => item.exercise_occurrence_key === exercise.exercise_occurrence_key); const done = exerciseItems.filter((item) => completed.some((result) => result.completion_item_key === item.completion_item_key)).length; return `<div class="today-progress-row"><span><strong>${escapeHtml(exercise.name)}</strong><small>${done}/${exerciseItems.length} 项</small></span><span class="today-row-status">${done === exerciseItems.length ? "✓" : "进行中"}</span></div>`; }).join("") : ""; const action = session.status === "in_progress" || session.status === "partial" ? `<button class="primary wide" data-action="${session.status === "partial" ? "continue" : "open-session"}">继续训练</button>` : `<button class="secondary wide" data-action="open-session">查看训练记录</button>`; return `<section class="today-page"><header class="today-app-header"><span></span><strong>今日</strong><span></span></header><div class="today-content"><p class="eyebrow">${state.today?.date || "今天"} · ${state.today?.timezone || ""}</p><h1>${escapeHtml(entry.title)}</h1><section class="today-progress-card"><div class="today-progress-head"><strong>${completed.length} / ${items.length || Math.round(1 / (fraction || 1))} 项完成</strong><span>${pct(fraction)}</span></div><div class="progress-line"><span style="width:${pct(fraction)}"></span></div>${rows || `<p class="muted">训练记录已保存。</p>`}</section>${action}${session.status === "completed" || session.status === "partial" ? `<button class="text-button wide" data-action="edit-session">校正记录</button>` : ""}</div></section>`; }

function sessionHeader(detail, showTimer = true) { return `<header class="session-header"><button class="session-header-side" data-action="minimize" aria-label="返回今日">‹</button><strong ${showTimer ? "data-session-elapsed" : ""}>${showTimer ? formatElapsed(detail) : "组间休息"}</strong><span aria-hidden="true"></span></header>`; }
...
function syncSessionClock() {
  const shouldRun = Boolean(state.sessionDetail?.status === "in_progress" && state.view === "today" && !state.endSheet);
...
function endSheet(detail) {
  ...
  const unfinishedMarkup = unfinished.length ? `<section class="end-unfinished"><h3>未完成项目</h3><ul>${unfinished.map((item, index) => `<li><span>${index + 1}</span><strong>${escapeHtml(exerciseName(detail, item))}${item.side === "none" ? "" : ` · ${item.side === "left" ? "left" : "right"}`}</strong></li>`).join("")}</ul></section>` : "";
...
function calendarView() {
  ...
  const rows = state.calendar.entries.map((entry) => {
    ...
    return `<button class="calendar-day${selectedClass}" data-action="calendar-select" data-date="${entry.date}" ${beforePlan ? "disabled" : ""}><span class="calendar-day-label">${weekdayLabels[entry.weekday]}</span><span class="calendar-day-date">${entry.date.slice(5)}</span><span class="calendar-day-summary">${entry.kind === "workout" ? escapeHtml(entry.title) : status.label}</span><span class="calendar-day-meta">${entry.kind === "workout" ? `${entry.module_count || 0} 个模块 · ${entry.estimated_duration_min} 分钟 · ${status.label}` : entry.kind === "rest" ? "恢复，不创建训练记录" : beforePlan ? "" : "未安排内容"}</span></button>`;
  }).join("");
...
function renderCalendarPrescription(prescription, detail) {
  ...
  return `<p class="muted">${prescription.blocks.length} 个训练模块 · ${prescription.title ? escapeHtml(prescription.title) : ""}</p>${prescription.blocks.map((block, blockIndex) => `<article class="prescription-block"><h4>${escapeHtml(block.title)}</h4>${block.exercises.map((exercise, exerciseIndex) => { const snapshotExercise = snapshotBlocks[blockIndex]?.exercises?.[exerciseIndex]; return `<div class="prescription-exercise"><strong>${escapeHtml(exercise.name)}</strong>${exercise.sets.map((set, index) => { const snapshotSet = snapshotExercise?.sets?.[index]; const actuals = detail && snapshotSet ? (detail.snapshot.completion_items || []).filter((item) => item.set_key === snapshotSet.set_key).map((item) => detail.completion_results.find((result) => result.completion_item_key === item.completion_item_key)?.actual ? `${item.side === "none" ? "" : `${item.side} `}${formatActual(detail.completion_results.find((result) => result.completion_item_key === item.completion_item_key).actual)}` : null).filter(Boolean) : []; return `<div class="prescription-set"><span>第 ${index + 1} 组 · ${formatTarget(set.target)}</span><span>${formatResistance(set.resistance)}${set.tempo ? ` · 节奏 ${escapeHtml(set.tempo)}` : ""}${set.rest_after_sec == null ? "" : ` · 休息 ${set.rest_after_sec} 秒`}</span>${detail ? `<small class="${actuals.length ? "actual" : "unfinished"}">${actuals.length ? `实际：${actuals.join("，")}` : "未完成"}</small>` : ""}</div>`; }).join("")}</div>`; }).join("")}</article>`).join("")}`;
}
...
function bind() {
  ...
  app.querySelectorAll("[data-action]").forEach((element) => element.addEventListener("click", () => action(element.dataset.action, element.dataset.index ?? element.dataset.exerciseKey ?? element.dataset.rpe, element.dataset.date)));
...
async function action(name, value, date) {
  try {
    ...
    if (name === "complete" || name === "save-adjust") return completeCurrent(); if (name === "previous") { state.focusIndex = Math.max(0, state.focusIndex - 1); state.adjust = false; return render(); } if (name === "next") { const max = state.sessionDetail?.snapshot?.completion_items?.length - 1 || 0; state.focusIndex = Math.min(max, state.focusIndex + 1); state.adjust = false; return render(); } if (name === "jump-item") { state.focusIndex = Number(value); state.progressOpen = false; state.adjust = false; state.restUntil = null; state.restNextIndex = null; return render(); } if (name === "toggle-adjust") { state.adjust = !state.adjust; return render(); } if (name === "toggle-progress") { state.progressOpen = !state.progressOpen; return render(); } if (name === "minimize") { stopSessionClock(); state.sessionDetail = null; state.progressOpen = false; state.adjust = false; state.feedbackOpen = null; state.restUntil = null; state.restNextIndex = null; state.endSheet = false; return refresh(); } if (name === "skip-rest") { state.focusIndex = state.restNextIndex ?? state.focusIndex; state.restUntil = null; state.restNextIndex = null; return render(); } if (name === "open-feedback") { state.feedbackOpen = value || null; return render(); } if (name === "close-feedback") { state.feedbackOpen = null; return render(); }
