...
- `migrations/0001_initial.sql`、`0002_state_revision.sql`、`0003_query_indexes.sql`、`0004_restore_session_date_guard.sql` 与 `0005_agent_token_lookup.sql` 覆盖 Athlete state 的唯一 email、初始化、乐观并发版本边界、Session 日期唯一性、Plan/Session/Exercise/Coach digest 查询投影，以及 Agent Token digest 查询投影；D1 binding 名为 `DB`。
...
1. 通过 Cloudflare Worker Secrets 写入两条准确的 normalized email、两个独立密码、随机 `AUTH_SESSION_SECRET` 和随机 `AGENT_TOKEN_SECRET`；禁止把这些值写入仓库或 GitHub Actions 日志。
