...
- `migrations/0001_initial.sql`、`0002_state_revision.sql` 与 `0003_query_indexes.sql` 覆盖 Athlete state 的唯一 email、初始化、乐观并发版本边界，以及 Plan/Session/Exercise/Coach digest 查询投影；D1 binding 名为 `DB`。
...
4. D1 migration 在空的 recovery fixture 上成功；`0003_query_indexes.sql` 的索引投影、Worker/D1 quota、日志/trace redaction、cache bypass 均用非敏感请求复核。
