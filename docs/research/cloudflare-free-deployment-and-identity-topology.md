...
Live account verification on 2026-07-31 confirmed an active Free `lagrangee.xyz` zone, Workers Free, authenticated Wrangler `4.115.0`, and no existing `workout` Worker, DNS record, or Custom Domain. Zero Trust is not onboarded and still shows **Get started**, so OTP, the two Athlete identities, Access seats, team configuration, application audience, and default-deny behavior remain deployment gates. The intended production host is `workout.lagrangee.xyz`.
...
- Bind `workout.lagrangee.xyz` as the sole production custom domain; disable `workers.dev` and Preview URLs.
