...
export function athleteExport(state, now) {
  ...
  const revisions = state.plan_revisions.map(/** @param {any} revision */ (revision) => ({ revision_key: revision.revision_key, revision_sequence: revision.revision_sequence, created_at: revision.created_at, effective_from: revision.effective_from, week: deepClone(revision.week) }));
  const firstPlan = state.plan_revisions.map(/** @param {any} revision */ (revision) => revision.effective_from).sort()[0];
  const schedule = firstPlan ? dateRange(firstPlan, today).map((date) => exportSchedule(state, date, now)).filter(Boolean) : [];
  const sessions = state.sessions.slice().sort(/** @param {any} a @param {any} b */ (a, b) => a.scheduled_date.localeCompare(b.scheduled_date) || a.session_key.localeCompare(b.session_key)).map(exportSession);
...
function exportSession(session) {
  const snapshot = {
    ...
    blocks: session.snapshot.blocks.map(/** @param {any} block */ (block) => ({ block_key: block.block_key, title: block.title, exercises: block.exercises.map(/** @param {any} exercise */ (exercise) => ({ exercise_occurrence_key: exercise.exercise_occurrence_key, exercise_key: exercise.exercise_key, name: exercise.name, category: exercise.category, side_mode: exercise.side_mode, sets: exercise.sets.map(/** @param {any} set */ (set) => ({ set_key: set.set_key, target: set.target, resistance: set.resistance, target_rir: set.target_rir, target_rpe: set.target_rpe, tempo: set.tempo, rest_after_sec: set.rest_after_sec, target_incline_percent: set.target_incline_percent })) })) })),
    completion_items: session.snapshot.completion_items.map(/** @param {any} item */ (item) => ({ completion_item_key: item.completion_item_key, exercise_occurrence_key: item.exercise_occurrence_key, set_key: item.set_key, side: item.side, target: item.target })),
  };
  return { session_key: session.session_key, scheduled_workout_key: session.scheduled_workout_key, scheduled_date: session.scheduled_date, timezone_at_session: session.timezone_at_session, title: session.title, status: session.status, completion_fraction: completionFraction(session), training_duration_sec: Math.round(trainingDuration(session)), session_rpe: session.session_rpe, note: session.note, skip_reason: session.skip_reason, snapshot, completion_results: session.completion_results.map(/** @param {any} result */ (result) => ({ completion_item_key: result.completion_item_key, actual: result.actual, resistance: result.resistance, rir: result.rir, completed_at: result.completed_at })), training_intervals: deepClone(session.training_intervals), exercise_feedback: deepClone(session.exercise_feedback), created_at: session.created_at, updated_at: session.updated_at };
}
