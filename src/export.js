...
export function athleteExport(state, now) {
  ...
  const revisions = state.plan_revisions.map((revision) => ({ revision_key: revision.revision_key, revision_sequence: revision.revision_sequence, created_at: revision.created_at, effective_from: revision.effective_from, week: deepClone(revision.week) }));
