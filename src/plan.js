...
export function effectiveRevision(state, date) {
  return state.plan_revisions
    .filter((revision) => revision.effective_from <= date)
    .sort((left, right) => right.revision_sequence - left.revision_sequence)[0] ?? null;
}
  ...
  slot.blocks.forEach((block, blockIndex) => {
    ...
    block.exercises.forEach((exercise, exerciseIndex) => {
      ...
      exercise.sets.forEach((set, setIndex) => {
        ...
        sides.forEach((side) => snapshot.completion_items.push({
...
export function scheduleEntry(state, date, now = new Date()) {
  const { revision, slot } = resolveSlot(state, date);
  const session = state.sessions.find((item) => item.scheduled_date === date) ?? null;
...
export function todayModel(state, now = new Date()) {
  ...
  const session = entry.session_key ? state.sessions.find((item) => item.session_key === entry.session_key) ?? null : null;
...
export function trainingDuration(session) {
  return session.training_intervals.reduce((total, interval) => interval.ended_at ? total + Math.max(0, (Date.parse(interval.ended_at) - Date.parse(interval.started_at)) / 1000) : total, 0);
}
...
export function sessionSummary(session) {
  return {
    ...
    exercise_keys: [...new Set(session.snapshot.blocks.flatMap((block) => block.exercises.map((exercise) => exercise.exercise_key)))],
...
export function planModel(state, now = new Date()) {
  ...
  const future = state.plan_revisions
    .filter((revision) => revision.effective_from > today && effectiveRevision(state, revision.effective_from)?.revision_key === revision.revision_key)
    .sort((left, right) => left.effective_from.localeCompare(right.effective_from))
    .map((revision) => ({ effective_from: revision.effective_from, week: deepClone(revision.week) }));
  return {
...
export function validatePlanForState(state, text, now = new Date()) {
  ...
  const current = effectiveRevision(state, result.value.effective_from);
  if (current && canonicalJson(current.week) === canonicalJson(result.value.week)) return { ok: false, errors: [{ path: "/week", message: "This package does not change the effective template" }] };
  return { ok: true, value: result.value, preview: packagePreview(state, result.value, now) };
}
...
export function appendPlanRevision(state, packageValue, now = new Date()) {
  const revision = {
    ...
    revision_sequence: Math.max(0, ...state.plan_revisions.map((item) => item.revision_sequence)) + 1,
    created_at: now.toISOString(),
    effective_from: packageValue.effective_from,
    week: deepClone(packageValue.week),
  };
  state.plan_revisions.push(revision);
  state.plan_revisions.sort((left, right) => left.revision_sequence - right.revision_sequence);
  return revision;
}
