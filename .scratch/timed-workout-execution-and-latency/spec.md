...
The execution surface pauses the action countdown, tempo cues, and displayed Session timer together when the Athlete pauses. Leaving the execution surface, hiding the page, pagehide, device lock, or opening the end form also closes the active Training Interval; returning to the Session shows it paused and requires an explicit `继续`. Resuming opens a new Training Interval, so time spent away is not counted. After a manual completion it enters the configured rest countdown, which remains skippable. A visible mute control silences cues without changing the recorded Actual Training Data.

The browser requests a screen Wake Lock only while the execution surface is visible and actively running. If the page loses visibility, the execution timer pauses and the Athlete must resume it; returning to the foreground does not silently restart timing. After the explicit `继续`, the app may request Wake Lock again. Home Screen Web App behavior on supported iOS versions is treated as the preferred iPhone path; older or unsupported browsers receive a clear fallback rather than a false guarantee.
...
22. As an Athlete, I want the final five seconds of rest to cue once per second and the end to use a distinct cue, so that I know when to prepare without confusing rest guidance with active-set tempo.
...
26. As an Athlete, I want Wake Lock to be requested again when I explicitly continue after returning to the foreground, so that a temporary visibility change does not silently restart my workout.
...
- During the five-second preparation phase, play a distinct restrained warm-up cue once per second. During the full fixed-duration action, play one tempo cue per second. Elevate the cue for the final three seconds and use a distinct end sound at zero. Use tones rather than speech synthesis for this spec.
...
- Pausing stops the action timer, cues, and displayed Session timer as one visible operation. The server closes the active Training Interval while retaining `in_progress`; resuming opens a new interval. Page visibility loss, pagehide, navigation away, and opening the end form use the same server-backed pause boundary. Ending a paused in-progress Session is valid and never counts the time spent paused.
- After a confirmed Completion Item save, start the existing configured rest countdown automatically. Rest remains skippable. During the final five seconds, play one cue per second; at zero play the distinct rest-complete cue. Rest cues do not become the active-set tempo stream.
...
- Request a screen Wake Lock only while the execution surface is visible and the Athlete is actively executing. If the page becomes hidden, pause execution and show a resumable state; after foreground recovery, remain paused until an explicit `继续`, then request Wake Lock again. Do not claim that the browser will continue running reliably in the background.
...
- The browser flow must prove that a timed Completion Item shows the five-second preparation phase, plays one warm-up cue per preparation second, then shows the fixed countdown with one cue per active second, elevated final-three-second cues, and a distinct zero/end cue.
...
- The browser flow must prove that Wake Lock is requested while visible, that foreground recovery remains paused until explicit continue, that continue requests Wake Lock again, and that visibility loss enters a paused/resumable state when the capability is unavailable or released. It must also prove that pagehide/navigation closes the server interval and excludes hidden time after resume.
...
- Redesigning the D1 state document, adding a separate pause table, or changing the existing Session Status values, correction rules, or immutable Training Plan Snapshot behavior. Pause/resume uses the existing Training Interval array and `in_progress` status.
