...
import { McpBridge, WorkoutApiClient } from "./bridge.mjs";
...
export async function runStdio({ input = process.stdin, output = process.stdout, bridge = createDefaultBridge() } = {}) {
  ...
  for await (const line of lines) {
    ...
    let message;
    try { message = JSON.parse(line); } catch { output.write(`${JSON.stringify(errorResponse(null, -32700, "Parse error"))}\n`); continue; }
    let response;
    try { response = await bridge.handleMessage(message); } catch (error) { response = hasId(message) ? errorResponse(message.id, -32000, error instanceof Error ? error.message : String(error)) : null; }
